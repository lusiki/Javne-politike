...
code-fold: false
echo: false
comments: false   # bez Hypothesis na Atlasu (izvjestaj-estetski M17)
format:
  html:
...
table = {
  ...
  const fmt = v => (v == null || Number.isNaN(+v)) ? "—" : (+v).toFixed(1).replace(".", ",");
...
chart = {
  ...
  if (spec.kind === "cofog") {
    ...
    plot = Plot.plot({
      ...
      color: {domain: ["HR","EU27_2020"], range: [gold, ink], legend: true},
      marks: [
        Plot.barX(rows, {

...
pri čemu ga jedino ograničava to da sponzor ne smije ostati na gubitku [@niskanen1971]. Intuicija je važna jer pokazuje da prekomjerna potrošnja nije pogreška u računu, nego ishod posve drugačijeg cilja. Sve dok ukupna korist tek pokriva ukupni trošak, birokrat ima razlog gurati uslugu dalje, pa ograničenje $B(Q) \ge C(Q)$ u ravnoteži postaje vezujuće i prelazi u jednakost $B(Q)=C(Q)$.
...
```{ojs}
//| echo: false
//| label: fig-javna-zaposlenost
//| fig-cap: "Zaposlenost u općoj državi kao udio u ukupnoj zaposlenosti, odabrane zemlje, 2023. (%). Zlatno je istaknuta Hrvatska, a isprekidana linija označava prosjek OECD-a. Izvor: OECD, Government at a Glance 2025."
//| fig-alt: "Vodoravni stupčasti graf udjela zaposlenih u općoj državi za devet zemalja. Norveška je na vrhu s 30,1 %, Japan na dnu s 4,9 %. Hrvatska je istaknuta zlatno s 22,8 %, iznad prosjeka OECD-a od 18,4 %."
{
  const data = [
  ...
  return Plot.plot({
    width: 780, height: 430, marginLeft: 86, marginRight: 50, marginTop: 28, marginBottom: 46,
    style: {fontSize: "12px", fontFamily: "Public Sans, system-ui, sans-serif", color: "#3A332D"},
...
::: {.content-visible when-format="pdf"}
```{r}
#| label: fig-javna-zaposlenost-print
#| echo: false
#| fig-cap: "Zaposlenost u općoj državi kao udio u ukupnoj zaposlenosti, odabrane zemlje, 2023. (%). Zlatno je istaknuta Hrvatska, a isprekidana linija označava prosjek OECD-a. Izvor: OECD, Government at a Glance 2025."
#| fig-alt: "Vodoravni stupčasti graf udjela zaposlenih u općoj državi; Norveška 30,1 %, Japan 4,9 %, Hrvatska istaknuta zlatno s 22,8 %, iznad prosjeka OECD-a od 18,4 %."
#| fig-width: 7.8
#| fig-height: 4.3
source("R/setup.R")

jz <- data.frame(
  z = c("Norveška","Švedska","Danska","Finska","Hrvatska","Francuska","Italija","Njemačka","Japan"),
  v = c(30.1, 28.1, 27.3, 25.2, 22.8, 20.7, 13.7, 11.5, 4.9),
  grp = c("ostalo","ostalo","ostalo","ostalo","HR","ostalo","ostalo","ostalo","ostalo"),
  stringsAsFactors = FALSE
)
jz$z <- factor(jz$z, levels = rev(jz$z))

ggplot(jz, aes(v, z, fill = grp)) +
  geom_col(width = 0.72) +
  geom_vline(xintercept = 18.4, linetype = "dashed", color = "#8C8C8C", linewidth = 0.5) +
  annotate("text", x = 18.4, y = 9, label = "OECD 18,4", hjust = -0.05, vjust = 0,
           size = 2.8, color = "#6B6357") +
  geom_text(aes(label = sub("\\.", ",", sprintf("%.1f", v))),
            hjust = -0.2, size = 2.9, color = "#3A332D") +
  scale_fill_manual(values = c("ostalo" = "#8C8C8C", "HR" = "#1A1A1A"), guide = "none") +
  scale_x_continuous(limits = c(0, 33)) +
  labs(x = "Zaposleni u općoj državi (% ukupne zaposlenosti)", y = NULL) +
  theme_pubfin()
```
:::

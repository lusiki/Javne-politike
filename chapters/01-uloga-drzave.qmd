...
```{ojs}
//| echo: false
//| label: fig-velicina-drzave
//| fig-cap: "Ukupni rashodi opće države u % BDP-a, Hrvatska i prosjek EU27, 2002.–2024. Zlatno je Hrvatska, zeleno prosjek Unije. Vrhunac u 2020. odražava pandemijski fiskalni odgovor. Izvor: Eurostat (gov_10a_main, sektor opće države S13)."
//| fig-alt: "Linijski graf ukupnih rashoda opće države u postotku BDP-a od 2002. do 2024. za Hrvatsku (zlatno) i prosjek EU27 (zeleno). Obje serije kreću se oko 45 do 51 % BDP-a, uz nagli skok u 2020. na 53,8 % za Hrvatsku i 52,9 % za EU."
{
  ...
  return Plot.plot({
    ...
    marginLeft: 52, marginRight: 28, marginTop: 30, marginBottom: 44,
    ...
    color: {domain: ["Hrvatska","EU27"], range: ["#C8942D","#4A6B5C"], legend: true},
    marks: [
      Plot.lineY(data, {x: "god", y: "v", stroke: "serija", strokeWidth: 2, curve: "monotone-x"}),
      Plot.dot(data, {x: "god", y: "v", fill: "serija", r: 2.5, tip: true}),
      Plot.text([{god: 2020, v: 53.8, t: "COVID 2020."}], {x: "god", y: "v", text: "t", dy: -12, dx: 6, textAnchor: "start", fontSize: 10, fill: "#6B6357"})
    ]
  });
}
```
...
```{r}
#| label: fig-velicina-drzave-print
#| echo: false
#| fig-cap: "Ukupni rashodi opće države u % BDP-a, Hrvatska i prosjek EU27, 2002.–2024. Zlatno je Hrvatska, zeleno prosjek Unije. Vrhunac u 2020. odražava pandemijski fiskalni odgovor. Izvor: Eurostat (gov_10a_main)."
#| fig-alt: "Linijski graf ukupnih rashoda opće države u postotku BDP-a od 2002. do 2024. za Hrvatsku (zlatno) i prosjek EU27 (zeleno), uz nagli skok u 2020."
#| fig-width: 7.8
#| fig-height: 4.4
source("R/setup.R")

god <- 2002:2024
hr  <- c(49.2,49.6,49.0,46.5,44.6,46.4,46.3,49.3,48.2,48.6,47.3,48.0,48.7,47.6,45.9,44.3,45.2,46.4,53.8,48.2,45.1,46.6,48.4)
eu  <- c(47.8,48.1,47.4,47.2,46.6,45.8,47.0,50.9,50.7,49.3,50.0,50.0,49.3,48.3,47.5,47.0,46.7,46.6,52.9,51.2,49.2,48.9,49.1)
df <- rbind(
  data.frame(god = god, v = hr, serija = "Hrvatska"),
  data.frame(god = god, v = eu, serija = "EU27")
)
df$serija <- factor(df$serija, levels = c("Hrvatska", "EU27"))

ggplot(df, aes(god, v, color = serija)) +
  geom_line(linewidth = 0.9) +
  geom_point(size = 1.2) +
  annotate("text", x = 2020, y = 55.2, label = "COVID 2020.", size = 2.8, color = "#6B6357") +
  scale_color_manual(values = c("Hrvatska" = "#1A1A1A", "EU27" = "#7A7A7A"), name = NULL) +
  scale_y_continuous(limits = c(40, 56)) +
  scale_x_continuous(breaks = seq(2002, 2024, 4)) +
  labs(x = NULL, y = "Rashodi opće države (% BDP-a)") +
  theme_pubfin()
```
:::

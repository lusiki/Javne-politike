...
//| fig-alt: "Dumbbell graf percentilnih rangova za šest dimenzija upravljanja. Za svaku dimenziju zlatna točka označava Hrvatsku, zelena Sloveniju, a Slovenija je desno (viša) na svih šest. Najveći je razmak na vladavini prava (Hrvatska 59, Slovenija 83) i kontroli korupcije (60 naspram 77)."
{
  const rows = [
  ...
  return Plot.plot({
    ...
    color: {domain: ["Hrvatska","Slovenija"], range: ["#C8942D","#4A6B5C"], legend: true},
    marks: [
      ...
      Plot.text(rows, {x: "si", y: "dim", text: d => d.si.toFixed(0), dy: -12, fontSize: 9, fill: "#4A6B5C"})
    ]
  });
}
```
...
#| fig-alt: "Dumbbell graf percentilnih rangova za šest dimenzija upravljanja; Hrvatska (zlatno) zaostaje za Slovenijom (zeleno) na svakoj, najviše na vladavini prava i kontroli korupcije."
#| fig-width: 7.8
#| fig-height: 4.0
source("R/setup.R")
...
ggplot(df) +
  ...
  geom_text(aes(x = si, y = dim, label = sprintf("%.0f", si)), vjust = -1.1, size = 2.6, color = "#4A6B5C") +
  scale_color_manual(values = c("Hrvatska" = "#C8942D", "Slovenija" = "#4A6B5C"), name = NULL) +

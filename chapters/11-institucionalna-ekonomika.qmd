---
title: "Nova institucionalna ekonomika"
---
...
//| echo: false
//| label: fig-wgi
//| fig-cap: "Kvaliteta upravljanja, percentilni rang (0–100, viši je bolji), Hrvatska i Slovenija, 2023. Slovenija nadmašuje Hrvatsku na svih šest dimenzija, a najveći zaostatak Hrvatske je na vladavini prava i kontroli korupcije. Izvor: Svjetska banka, Worldwide Governance Indicators (ažuriranje 2024.)."
//| fig-alt: "Dumbbell graf percentilnih rangova za šest dimenzija upravljanja. Za svaku dimenziju zlatna točka označava Hrvatsku, sivoplava Sloveniju, a Slovenija je desno (viša) na svih šest. Najveći je razmak na vladavini prava (Hrvatska 59, Slovenija 83) i kontroli korupcije (60 naspram 77)."
{
  const rows = [
    {dim:"Vladavina prava", hr:59.4, si:82.5},
    {dim:"Kontrola korupcije", hr:59.9, si:77.4},
    {dim:"Glas i odgovornost", hr:65.7, si:82.8},
    {dim:"Politička stabilnost", hr:67.8, si:75.8},
    {dim:"Kvaliteta regulacije", hr:72.2, si:75.0},
    {dim:"Učinkovitost vlasti", hr:75.5, si:81.1}
  ];
  const order = rows.map(d => d.dim);
  return Plot.plot({
    width: 800, height: 400,
    marginLeft: 150, marginRight: 44, marginTop: 30, marginBottom: 46,
    style: {fontSize: "12px", fontFamily: "Public Sans, system-ui, sans-serif", color: "#3A332D"},
    x: {label: "Percentilni rang (0–100) →", domain: [0, 100], grid: true},
    y: {label: null, domain: order},
    color: {domain: ["Hrvatska","Slovenija"], range: ["#C8942D","#4A5568"], legend: true},
    marks: [
      Plot.link(rows, {x1: "hr", x2: "si", y: "dim", stroke: "#C9C3B8", strokeWidth: 2.5}),
      Plot.dot(rows, {x: "hr", y: "dim", fill: () => "Hrvatska", r: 6, stroke: "white", strokeWidth: 1, tip: true}),
      Plot.dot(rows, {x: "si", y: "dim", fill: () => "Slovenija", r: 6, stroke: "white", strokeWidth: 1, tip: true}),
      Plot.text(rows, {x: "hr", y: "dim", text: d => d.hr.toFixed(0), dy: -12, fontSize: 9, fill: "#9A6F38"}),
      Plot.text(rows, {x: "si", y: "dim", text: d => d.si.toFixed(0), dy: -12, fontSize: 9, fill: "#4A5568"})
    ]
  });
}
```
...
#| label: fig-wgi-print
#| echo: false
#| fig-cap: "Kvaliteta upravljanja, percentilni rang (0–100, viši je bolji), Hrvatska i Slovenija, 2023. Slovenija nadmašuje Hrvatsku na svih šest dimenzija. Izvor: Svjetska banka, Worldwide Governance Indicators (ažuriranje 2024.)."
#| fig-alt: "Dumbbell graf percentilnih rangova za šest dimenzija upravljanja; Hrvatska (zlatno) zaostaje za Slovenijom (sivoplavo) na svakoj, najviše na vladavini prava i kontroli korupcije."
#| fig-width: 7.8
#| fig-height: 4.0
source("R/setup.R")

df <- data.frame(
  dim = c("Vladavina prava","Kontrola korupcije","Glas i odgovornost",
          "Politička stabilnost","Kvaliteta regulacije","Učinkovitost vlasti"),
  hr  = c(59.4, 59.9, 65.7, 67.8, 72.2, 75.5),
  si  = c(82.5, 77.4, 82.8, 75.8, 75.0, 81.1),
  stringsAsFactors = FALSE
)
df$dim <- factor(df$dim, levels = rev(df$dim))

ggplot(df) +
  geom_segment(aes(x = hr, xend = si, y = dim, yend = dim),
               color = "#C9C3B8", linewidth = 1.3) +
  geom_point(aes(x = hr, y = dim, color = "Hrvatska"), size = 3) +
  geom_point(aes(x = si, y = dim, color = "Slovenija"), size = 3) +
  geom_text(aes(x = hr, y = dim, label = sprintf("%.0f", hr)), vjust = -1.1, size = 2.6, color = "#9A6F38") +
  geom_text(aes(x = si, y = dim, label = sprintf("%.0f", si)), vjust = -1.1, size = 2.6, color = "#4A5568") +
  scale_color_manual(values = c("Hrvatska" = "#C8942D", "Slovenija" = "#4A5568"), name = NULL) +
  scale_x_continuous(limits = c(0, 100)) +
  labs(x = "Percentilni rang (0–100, viši je bolji)", y = NULL) +
  theme_pubfin()
```
:::
...
gdje je $p\,q$ ono što bi se platilo u udžbeničkom svijetu bez trenja, a zagrada sve ono što razmjenu čini skupljom u stvarnom svijetu [@coase1937; @north1990]. Northova teza da bolje institucije snižavaju troškove razmjene u ovom zapisu znači jednostavno da pomak prema pouzdanijim pravilima gura drugu zagradu prema nuli, a da cijena $p\,q$ pritom ostaje ista. Ako je institucionalni okvir slab, ti troškovi mogu postati toliko veliki da razmjena postaje preskupa ili prerizična. U tom slučaju tržište formalno postoji, ali ne funkcionira dobro jer su troškovi korištenja tržišta previsoki.
...
pa uvjet prvog reda traži da granična vrijednost ulaganja $v'(I^*)$ bude $1/s$, dakle iznad društveno poželjne razine 1 [@williamson1985]. Budući da je $v$ konkavna, viša tražena granična vrijednost znači manje uloženog $I$, što je upravo poddulaganje. Vertikalna integracija, dakle proizvodnja umjesto kupnje, uklanja taj problem jer obje strane stavlja pod istu upravu i time vraća $s$ prema 1, čime se i granica između poduzeća i tržišta pomiče točno ondje gdje specifičnost ulaganja čini tržišnu razmjenu preskupom.
...
gdje je $r$ averzija agenta prema riziku, $\sigma^2$ varijanca šuma u mjerenju ishoda, a $m$ produktivnost truda [@holmstrommilgrom1987]. Optimalna osjetljivost plaće $b^*$ pada kako ishod postaje bučniji (veći $\sigma^2$) ili agent nesklniji riziku (veći $r$), jer jaki poticaji tada agenta izlažu riziku koji nije pod njegovom kontrolom i koji se mora skupo nadoknaditi. To objašnjava zašto u javnom sektoru, gdje su učinci često teško mjerljivi i okruženi šumom, snažni poticaji vezani uz rezultat često nisu izvedivi, pa prevladavaju fiksne plaće i nadzor postupka umjesto ishoda.

...
bar_col  <- function(v) ifelse(v <= 1, "#C53030", ifelse(v == 2, "#C8942D", "#1C7C54"))
# Boja naslovne ocjene po istom pragu kao u OJS-u.
head_col <- if (pct >= 80) "#1C7C54" else if (pct >= 55) "#C8942D" else "#C53030"

ggplot(dims, aes(x = v, y = q)) +
  geom_col(fill = bar_col(dims$v), width = 0.7) +
  geom_vline(xintercept = 0, color = "#3A332D", linewidth = 0.6) +

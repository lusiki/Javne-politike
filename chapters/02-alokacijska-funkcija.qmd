...
palette <- c("#A9C3DA", "#7FA3C6", "#5683B0", "#3D6B99", "#2D5A8E", "#1E3F63")
...
ggplot() +
  # Baseline
  geom_hline(yintercept = 0, color = "#C9C3B8", linewidth = 0.4) +
  ...
  geom_line(data = msb, aes(Q, P), color = "#1C1916", linewidth = 1.2) +
  # MC linija (vodoravna)
  geom_line(data = mcLine, aes(Q, P), color = "#6B1F26", linewidth = 1.0) +
  # Isprekidane vodilice
  geom_hline(yintercept = P_star, color = "#6B6357", linetype = "dashed", linewidth = 0.4) +
  annotate("segment", x = Qm,    xend = Qm,    y = 0, yend = P_star,
           color = "#6B6357", linetype = "dashed", linewidth = 0.4) +
...
ggplot() +
  ...
  geom_polygon(data = dwlTri, aes(Q, P),
               fill = "#D8B5B3", alpha = 0.65,
               color = "#6B1F26", linewidth = 0.4) +
  # Krivulje
  geom_line(data = demand, aes(Q, P), color = "#2D5A8E", linewidth = 1.1) +
  geom_line(data = mpc,    aes(Q, P), color = "#4A6B5C", linewidth = 0.9) +
  geom_line(data = msc,    aes(Q, P), color = "#6B1F26", linewidth = 0.9) +
  # Isprekidane vodilje
  geom_hline(yintercept = Pstar, color = "#6B6357", linetype = "dashed", linewidth = 0.4) +
  geom_hline(yintercept = Pmpc,  color = "#6B6357", linetype = "dashed", linewidth = 0.4) +
...
ggplot() +
  ...
  geom_polygon(data = dwlTri, aes(Q, P),
               fill = "#E8D6B8", alpha = 0.65,
               color = "#C8985E", linewidth = 0.4) +
  # krivulje
  geom_line(data = mpb,    aes(Q, P), color = "#2D5A8E", linewidth = 1.0) +
  geom_line(data = msb,    aes(Q, P), color = "#2B6CB0", linewidth = 0.8) +
  geom_line(data = supply, aes(Q, P), color = "#4A6B5C", linewidth = 0.8) +
  # isprekidane vodilje
  geom_hline(yintercept = Pstar, linetype = "dashed", linewidth = 0.4, color = "#6B6357") +
  geom_hline(yintercept = Pm,    linetype = "dashed", linewidth = 0.4, color = "#6B6357") +
  geom_segment(aes(x = Qstar, xend = Qstar, y = 0, yend = Pstar),
               linetype = "dashed", linewidth = 0.4, color = "#6B6357") +
  geom_segment(aes(x = Qm, xend = Qm, y = 0, yend = MSB_Qm),
               linetype = "dashed", linewidth = 0.4, color = "#6B6357") +
  # ravnotežne točke
  geom_point(aes(x = Qm,    y = Pm),    size = 2.6, shape = 21, fill = "#2D5A8E", color = "white", stroke = 1) +
  geom_point(aes(x = Qstar, y = Pstar), size = 2.6, shape = 21, fill = "#2B6CB0", color = "white", stroke = 1) +
  # oznake krivulja
  annotate("text", x = Qmax * 0.82, y = a_d - b_d * Qmax * 0.82,
           label = sprintf("D = MPB (|εD|=%.2f)", eps_d_pos),
           color = "#2D5A8E", fontface = "bold", size = 3.5, vjust = -0.6) +
  annotate("text", x = Qmax * 0.7, y = a_d + b - b_d * Qmax * 0.7,
           label = sprintf("MSB (b=%d)", as.integer(b)),
           color = "#2B6CB0", fontface = "bold", size = 3.5, vjust = -0.6) +
  annotate("text", x = Qmax * 0.82, y = c_s + b_s * Qmax * 0.82,
           label = sprintf("S (εS=%.2f)", eps_s_pos),
           color = "#4A6B5C", fontface = "bold", size = 3.5, vjust = -0.6) +
  # oznake cijena i količina na osima
  annotate("text", x = 1, y = Pstar, label = sprintf("P* = %.1f", Pstar),
           color = "#2B6CB0", size = 3.2, hjust = 0, vjust = -0.6) +
  annotate("text", x = 1, y = Pm, label = sprintf("P_M = %.1f", Pm),
           color = "#2D5A8E", size = 3.2, hjust = 0, vjust = 1.4) +
  annotate("text", x = Qm, y = 2, label = sprintf("Q_M = %.1f", Qm),
           color = "#2D5A8E", size = 3.2, hjust = 0.5) +
  annotate("text", x = Qstar, y = 2, label = sprintf("Q* = %.1f", Qstar),
           color = "#2B6CB0", size = 3.2, hjust = 0.5) +
  # ispis propuštene koristi (DWL)
  annotate("text", x = 2, y = Pmax * 0.97,
           label = sprintf("Propuštena korist (DWL) = %.1f", dwl),
           color = "#9A6F38", fontface = "bold", size = 4.2, hjust = 0) +
  scale_x_continuous(limits = c(0, Qmax), expand = c(0, 0)) +
  scale_y_continuous(limits = c(0, Pmax), expand = c(0, 0)) +
...
ggplot() +
  ...
  geom_hline(yintercept = 0, color = "#C9C3B8") +
  # Krivulje
  geom_line(data = demand, aes(Q, P), color = "#2D5A8E", linewidth = 1.0) +
  geom_line(data = sLow,   aes(Q, P), color = "#4A6B5C", linewidth = 0.8) +
  geom_line(data = sHigh,  aes(Q, P), color = "#4A6B5C", linewidth = 0.8,
            linetype = "dashed") +
  ...
  geom_hline(yintercept = P_high, color = "#6B6357", linetype = "dashed", linewidth = 0.4) +
  geom_hline(yintercept = P_low,  color = "#6B6357", linetype = "dashed", linewidth = 0.4) +
  annotate("segment", x = Q1, xend = Q1, y = 0, yend = P_high,
           color = "#6B6357", linetype = "dashed", linewidth = 0.4) +
  annotate("segment", x = Q2, xend = Q2, y = 0, yend = P_low,
           color = "#6B6357", linetype = "dashed", linewidth = 0.4) +
  # Ravnotežne točke
  annotate("point", x = Q1, y = P_high, size = 3, color = "#1C1916") +
  annotate("point", x = Q2, y = P_low,  size = 3, color = "#6B1F26") +
  # Oznake krivulja
  annotate("text", x = Qmax * 0.82, y = a_d - b_d * Qmax * 0.82,
           label = sprintf("D (|eD|=%.2f)", eps_d_asi),
           color = "#2D5A8E", fontface = "bold", size = 3.6, vjust = -0.6) +
  annotate("text", x = Qmax * 0.82, y = c_s + b_s * Qmax * 0.82,
           label = sprintf("S_loša (eS=%.2f)", eps_s_asi),
           color = "#4A6B5C", fontface = "bold", size = 3.6, vjust = -0.6) +
...
ggplot() +
  ...
  # Krivulje
  geom_line(data = demand, aes(Q, P), color = "#2D5A8E", linewidth = 1.0) +
  geom_line(data = mr, aes(Q, P), color = "#3A332D", linewidth = 0.8,
            linetype = "longdash") +
  geom_line(data = mc, aes(Q, P), color = "#4A6B5C", linewidth = 0.8) +
  # Isprekidane vodilje za cijene i količine
  geom_hline(yintercept = Pm, color = "#6B6357", linetype = "dashed",
             linewidth = 0.4) +
  geom_hline(yintercept = Pc, color = "#6B6357", linetype = "dashed",
             linewidth = 0.4) +
  ...
  annotate("text", x = 2, y = Pmax * 0.91,
           ...
           color = "#1C1916", fontface = "bold", size = 3.8, hjust = 0) +
  scale_x_continuous(limits = c(0, Qmax), expand = c(0, 0)) +
  scale_y_continuous(limits = c(0, Pmax), expand = c(0, 0)) +
  labs(x = "Količina (Q)", y = "Cijena (P)")
```

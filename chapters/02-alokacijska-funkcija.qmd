...
## Javna dobra: problem slobodnog jahača
...
//| echo: false
{
  ...
  return Plot.plot({
    ...
    marks: [
      ...
      Plot.line(msb, {x: "Q", y: "P", stroke: "#171411", strokeWidth: 3}),

      // MC line (horizontal)
      Plot.line(mcLine, {x: "Q", y: "P", stroke: "#9C302B", strokeWidth: 2.5}),

      // Dashed guides
      Plot.ruleY([P_star], {stroke: "#9F907B", strokeDasharray: "4,3"}),
      Plot.ruleX([Qm],    {y1: 0, y2: P_star, stroke: "#9F907B", strokeDasharray: "4,3"}),
      Plot.ruleX([Qstar], {y1: 0, y2: P_star, stroke: "#9F907B", strokeDasharray: "4,3"}),

      // Equilibrium dots
      Plot.dot([{Q: Qm,    P: P_star}],
        {x: "Q", y: "P", r: 5, fill: "#9C302B", stroke: "white", strokeWidth: 2}),
      Plot.dot([{Q: Qstar, P: P_star}],
        {x: "Q", y: "P", r: 5, fill: "#171411", stroke: "white", strokeWidth: 2}),
      ...
      Plot.text([{x: Qm, y: 3, label: `Q_M = ${Qm.toFixed(1)}`}],
        {x: "x", y: "y", text: "label", textAnchor: "middle",
         fontSize: 11, fill: "#9C302B"}),
      Plot.text([{x: Qstar, y: 3, label: `Q* = ${Qstar.toFixed(1)}`}],
        {x: "x", y: "y", text: "label", textAnchor: "middle",
         fontSize: 11, fill: "#171411"}),
      ...
      Plot.text([{x: 2, y: Pmax * 0.97,
        ...
        {x: "x", y: "y", text: "label", textAnchor: "start",
         fontSize: 14, fill: "#171411", fontWeight: 700})
    ]
  });
}
```

## Eksternalije: kada privatne odluke imaju društvene posljedice

Drugi veliki oblik tržišnog neuspjeha su **eksternalije**. Eksternalija nastaje kada odluka jednog pojedinca ili poduzeća utječe na dobrobit drugih, a taj se učinak ne odražava u tržišnoj cijeni. Drugim riječima, akter koji donosi odluku ne snosi sve troškove ili ne dobiva sve koristi koje njegova odluka stvara za društvo (Pigou, 1920; Baumol i Oates, 1988).
  ...
  return Plot.plot({
    ...
    marks: [
      ...
      Plot.line([...dwlTri, dwlTri[0]],
        {x: "Q", y: "P", stroke: "#9C302B", strokeWidth: 1,
         fill: "#E3B5B0", fillOpacity: 0.65, curve: "linear"}),

      // Curves
      Plot.line(demand, {x: "Q", y: "P", stroke: "#2D5A8E", strokeWidth: 2.5}),
      Plot.line(mpc,    {x: "Q", y: "P", stroke: "#2F7A5C", strokeWidth: 2}),
      Plot.line(msc,    {x: "Q", y: "P", stroke: "#9C302B", strokeWidth: 2}),

      // Dashed guides
      Plot.ruleY([Pstar], {stroke: "#9F907B", strokeDasharray: "4,3"}),
      Plot.ruleY([Pmpc],  {stroke: "#9F907B", strokeDasharray: "4,3"}),
      Plot.ruleX([Qstar], {y1: 0, y2: Pstar, stroke: "#9F907B", strokeDasharray: "4,3"}),
      Plot.ruleX([Qm],    {y1: 0, y2: Pmpc,  stroke: "#9F907B", strokeDasharray: "4,3"}),

      // Equilibrium dots
      Plot.dot([{Q: Qm,    P: Pmpc}],
        {x: "Q", y: "P", r: 5, fill: "#2F7A5C", stroke: "white", strokeWidth: 2}),
      ...
      Plot.text([{x: Qmax * 0.7, y: c_s + e + b_s * Qmax * 0.7, label: `MSC (e=${e})`}],
        {x: "x", y: "y", text: "label", fill: "#9C302B", fontSize: 12, fontWeight: 600, dy: -8}),

      // Price & quantity labels on axes
      Plot.text([{x: 1, y: Pstar, label: `P* = ${Pstar.toFixed(1)}`}],
        {x: "x", y: "y", text: "label", textAnchor: "start", dy: -7, fontSize: 11, fill: "#9C302B"}),
      Plot.text([{x: 1, y: Pmpc, label: `P_MPC = ${Pmpc.toFixed(1)}`}],
        {x: "x", y: "y", text: "label", textAnchor: "start", dy: 12, fontSize: 11, fill: "#2F7A5C"}),
      Plot.text([{x: Qstar, y: 2, label: `Q* = ${Qstar.toFixed(1)}`}],
        {x: "x", y: "y", text: "label", textAnchor: "middle", dy: 0, fontSize: 11, fill: "#9C302B"}),
      Plot.text([{x: Qm, y: 2, label: `Q_M = ${Qm.toFixed(1)}`}],
        {x: "x", y: "y", text: "label", textAnchor: "middle", dy: 0, fontSize: 11, fill: "#2F7A5C"}),
...
//| echo: false
{
  ...
  return Plot.plot({
    ...
    marks: [
      ...
      Plot.line([...dwlTri, dwlTri[0]],
        {x: "Q", y: "P", stroke: "#C57832", strokeWidth: 1,
         fill: "#F0DFC0", fillOpacity: 0.65, curve: "linear"}),
      ...
      // Dashed guides
      Plot.ruleY([Pstar], {stroke: "#9F907B", strokeDasharray: "4,3"}),
      Plot.ruleY([Pm],    {stroke: "#9F907B", strokeDasharray: "4,3"}),
      Plot.ruleX([Qstar], {y1: 0, y2: Pstar, stroke: "#9F907B", strokeDasharray: "4,3"}),
      Plot.ruleX([Qm],    {y1: 0, y2: MSB_Qm, stroke: "#9F907B", strokeDasharray: "4,3"}),
...
## Asimetrične informacije: kada jedna strana zna više od druge
...
//| echo: false
{
  ...
  return Plot.plot({
    ...
    marks: [
      ...
      // Curves
      Plot.line(demand, {x: "Q", y: "P", stroke: "#2D5A8E", strokeWidth: 2.5}),
      Plot.line(sLow,   {x: "Q", y: "P", stroke: "#2F7A5C", strokeWidth: 2}),
      Plot.line(sHigh,  {x: "Q", y: "P", stroke: "#2F7A5C", strokeWidth: 2, strokeDasharray: "6,4"}),
      ...
      Plot.link(
        ...
        {x1: "x1", y1: "y1", x2: "x2", y2: "y2",
         stroke: "#9C302B", strokeWidth: 2, markerEnd: "arrow"}),

      // Dashed price/quantity guides
      Plot.ruleY([P_high], {stroke: "#9F907B", strokeDasharray: "4,3"}),
      Plot.ruleY([P_low],  {stroke: "#9F907B", strokeDasharray: "4,3"}),
      Plot.ruleX([Q1],     {y1: 0, y2: P_high, stroke: "#9F907B", strokeDasharray: "4,3"}),
      Plot.ruleX([Q2],     {y1: 0, y2: P_low,  stroke: "#9F907B", strokeDasharray: "4,3"}),
      ...
      Plot.text([{x: 1, y: P_high, label: `P_visoka = ${P_high.toFixed(1)}`}],
        {x: "x", y: "y", text: "label", textAnchor: "start", dy: -7, fontSize: 11, fill: "#171411"}),
      Plot.text([{x: 1, y: P_low, label: `P_niska = ${P_low.toFixed(1)}`}],
        {x: "x", y: "y", text: "label", textAnchor: "start", dy: 12, fontSize: 11, fill: "#9C302B"}),

      // Quantity labels
      Plot.text([{x: Q1, y: 2, label: `Q₁ = ${Q1.toFixed(1)}`}],
        {x: "x", y: "y", text: "label", textAnchor: "middle", fontSize: 11, fill: "#171411"}),
      Plot.text([{x: Q2, y: 2, label: `Q₂ = ${Q2.toFixed(1)}`}],
        {x: "x", y: "y", text: "label", textAnchor: "middle", fontSize: 11, fill: "#9C302B"}),
...
//| echo: false
{
  ...
  return Plot.plot({
    ...
    marks: [
      ...
      // Cost curves
      Plot.line(cFull, {x: "e", y: "C", stroke: "#9C302B", strokeWidth: 2.5, strokeDasharray: "6,4"}),
      Plot.line(cIns,  {x: "e", y: "C", stroke: "#171411", strokeWidth: 2.5}),

      // Dashed verticals at e* and eM
      Plot.ruleX([eStar],   {y1: 0, y2: 1 + k * eStar * eStar, stroke: "#9F907B", strokeDasharray: "4,3"}),
      Plot.ruleX([eMshown], {y1: 0, y2: 1 + k * (1 - theta) * eMshown * eMshown,
                             stroke: "#9F907B", strokeDasharray: "4,3"}),

      // Equilibrium dots: each on the curve the agent actually faces
      Plot.dot([{e: eStar, C: 1 + k * eStar * eStar}],
        {x: "e", y: "C", r: 5, fill: "#9C302B", stroke: "white", strokeWidth: 2}),
      Plot.dot([{e: eMshown, C: 1 + k * (1 - theta) * eMshown * eMshown}],
        {x: "e", y: "C", r: 5, fill: "#171411", stroke: "white", strokeWidth: 2}),

      // Curve labels
      Plot.text([{x: emax * 0.78, y: 1 + k * (emax * 0.78) * (emax * 0.78), label: "Bez osiguranja"}],
        {x: "x", y: "y", text: "label", fill: "#9C302B", fontSize: 12, fontWeight: 700,
         dy: -10, textAnchor: "end"}),
      Plot.text([{x: emax * 0.88, y: 1 + k * (1 - theta) * (emax * 0.88) * (emax * 0.88),
                  label: "S osiguranjem (moralni hazard)"}],
        {x: "x", y: "y", text: "label", fill: "#171411", fontSize: 12, fontWeight: 700,
         dy: -10, textAnchor: "end"}),
      ...
      Plot.text([{x: 0.2, y: Cmax * 0.91,
        ...
        {x: "x", y: "y", text: "label", textAnchor: "start",
         fontSize: 13, fill: "#9C302B", fontWeight: 600})
    ]
  });
}
```
...
## Tržišna moć i monopol: kada konkurencija ne funkcionira
...
//| echo: false
{
  ...
  return Plot.plot({
    ...
    marks: [
      ...
      // Curves
      Plot.line(demand, {x: "Q", y: "P", stroke: "#2D5A8E", strokeWidth: 2.5}),
      Plot.line(mr,     {x: "Q", y: "P", stroke: "#5A4F3F", strokeWidth: 2, strokeDasharray: "5,4"}),
      Plot.line(mc,     {x: "Q", y: "P", stroke: "#2F7A5C", strokeWidth: 2}),

      // Dashed guides for prices and quantities
      Plot.ruleY([Pm], {stroke: "#9F907B", strokeDasharray: "4,3"}),
      Plot.ruleY([Pc], {stroke: "#9F907B", strokeDasharray: "4,3"}),
      Plot.ruleX([Qm], {y1: 0, y2: Pm, stroke: "#9F907B", strokeDasharray: "4,3"}),
      Plot.ruleX([Qc], {y1: 0, y2: Pc, stroke: "#9F907B", strokeDasharray: "4,3"}),
      ...
      Plot.text([{x: Qc, y: 2, label: `Q_C = ${Qc.toFixed(1)}`}],
        {x: "x", y: "y", text: "label", textAnchor: "middle", fontSize: 11, fill: "#2F7A5C"}),
...
## Zajednički resursi: kada je problem pretjerano korištenje
...
## Meritorna dobra: prijelaz između učinkovitosti i društvenih vrijednosti
...
## Zaključno: alokacijska funkcija kao prvi argument za državu
...
Zato je najvažnija pouka alokacijske funkcije sljedeća: država je potrebna ne zato da zamijeni tržište, nego zato da ispravi situacije u kojima tržište samo ne može proizvesti društveno učinkovit ishod.

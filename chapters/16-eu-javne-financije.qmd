---
title: "Javne financije Europske unije"
---
...
```{ojs}
//| echo: false
//| label: fig-mff-naslovi
//| fig-cap: "Višegodišnji financijski okvir EU 2021.–2027. po naslovima, udio u ukupnim obvezama (cijene 2018.). Kohezija te prirodni resursi i poljoprivreda zajedno čine oko dvije trećine. Ukupno 1.074 mlrd. € (cijene 2018.), odnosno 1.211 mlrd. € u tekućim cijenama. Izvor: Europski parlament, Vijeće EU."
//| fig-alt: "Vodoravni stupčasti graf udjela sedam naslova višegodišnjeg financijskog okvira EU. Kohezija 35 % i prirodni resursi 33 % dominiraju, slijede jedinstveno tržište 12 %, susjedstvo 9 %, uprava 7 %, migracije 2 % i obrana 1 %."
{
  const data = [
    {h:"Kohezija, otpornost i vrijednosti", v:35.2, grp:"glavno"},
    {h:"Prirodni resursi i okoliš (uklj. ZPP)", v:33.2, grp:"glavno"},
    {h:"Jedinstveno tržište, inovacije, digitalno", v:12.4, grp:"ostalo"},
    {h:"Susjedstvo i svijet", v:9.2, grp:"ostalo"},
    {h:"Europska javna uprava", v:6.8, grp:"ostalo"},
    {h:"Migracije i upravljanje granicama", v:2.1, grp:"ostalo"},
    {h:"Sigurnost i obrana", v:1.2, grp:"ostalo"}
  ];
  return Plot.plot({
    width: 780, height: 320, marginLeft: 232, marginRight: 44, marginTop: 18, marginBottom: 42,
    style: {fontSize: "12px", fontFamily: "Public Sans, system-ui, sans-serif", color: "#3A332D", background: "#F2EDE3"},
    x: {label: "% obveza okvira 2021.–2027. →", domain: [0, 40], grid: true},
    y: {label: null, domain: data.map(d => d.h)},
    color: {domain: ["glavno","ostalo"], range: ["#4A6B5C","#4A5568"]},
    marks: [
      Plot.barX(data, {x: "v", y: "h", fill: "grp", tip: true}),
      Plot.text(data, {x: "v", y: "h", text: d => d.v.toFixed(1).replace(".", ",") + " %", dx: 4, textAnchor: "start", fontSize: 10, fill: "#3A332D"})
    ]
  });
}
```
...
```{r}
#| label: fig-mff-naslovi-print
#| echo: false
#| fig-cap: "Višegodišnji financijski okvir EU 2021.–2027. po naslovima, udio u ukupnim obvezama (cijene 2018.). Kohezija te prirodni resursi i poljoprivreda zajedno čine oko dvije trećine. Ukupno 1.074 mlrd. € (cijene 2018.). Izvor: Europski parlament, Vijeće EU."
#| fig-alt: "Vodoravni stupčasti graf udjela sedam naslova MFF-a EU; kohezija 35 % i prirodni resursi 33 % dominiraju."
#| fig-width: 7.8
#| fig-height: 3.3
source("R/setup.R")
...
mff <- data.frame(
  h = c("Kohezija, otpornost i vrijednosti","Prirodni resursi i okoliš (uklj. ZPP)",
        "Jedinstveno tržište, inovacije, digitalno","Susjedstvo i svijet",
        "Europska javna uprava","Migracije i upravljanje granicama","Sigurnost i obrana"),
  v = c(35.2,33.2,12.4,9.2,6.8,2.1,1.2),
  grp = c("glavno","glavno","ostalo","ostalo","ostalo","ostalo","ostalo"),
  stringsAsFactors = FALSE
)
mff$h <- factor(mff$h, levels = rev(mff$h))
ggplot(mff, aes(v, h, fill = grp)) +
  geom_col(width = 0.74) +
  geom_text(aes(label = paste0(sub("\\.", ",", sprintf("%.1f", v)), " %")), hjust = -0.15, size = 2.8, color = "#3A332D") +
  scale_fill_manual(values = c("glavno"="#333333","ostalo"="#8C8C8C"), guide = "none") +
  scale_x_continuous(limits = c(0, 42)) +
  labs(x = "% obveza okvira 2021.–2027.", y = NULL)
```
:::
...
```{ojs}
//| echo: false
//| label: fig-neto-pozicija
//| fig-cap: "Neto proračunski položaj odabranih članica EU, euro po stanovniku, 2023. Pozitivno su neto primatelji (zeleno), negativno neto uplatitelji (crveno). Hrvatska (zlatno) je među najvećim neto primateljima po stanovniku u Uniji. Izvor: Europska komisija; izračuni Deutsche Bundesbank i IW Köln."
//| fig-alt: "Vodoravni stupčasti graf neto položaja po stanovniku za pet zemalja. Hrvatska +619 € i Rumunjska +307 € su neto primatelji, Nizozemska −350 € i Francuska −30 € neto uplatitelji."
{
  const data = [
    {z:"Hrvatska", v:619, grp:"HR"},
    {z:"Rumunjska", v:307, grp:"prim"},
    {z:"Poljska", v:191, grp:"prim"},
    {z:"Francuska", v:-30, grp:"upl"},
    {z:"Nizozemska", v:-350, grp:"upl"}
  ];
  return Plot.plot({
    width: 780, height: 280, marginLeft: 92, marginRight: 60, marginTop: 18, marginBottom: 46,
    style: {fontSize: "12px", fontFamily: "Public Sans, system-ui, sans-serif", color: "#3A332D", background: "#F2EDE3"},
    x: {label: "Neto položaj, € po stanovniku (2023.) →", domain: [-430, 730], grid: true},
    y: {label: null, domain: data.map(d => d.z)},
    color: {domain: ["HR","prim","upl"], range: ["#C8942D","#4A6B5C","#6B1F26"]},
    marks: [
      Plot.barX(data, {x: "v", y: "z", fill: "grp", tip: true}),
      Plot.ruleX([0], {stroke: "#3A332D"}),
      Plot.text(data, {x: "v", y: "z", text: d => (d.v > 0 ? "+" : "") + d.v + " €", dx: d => d.v > 0 ? 4 : -4, textAnchor: d => d.v > 0 ? "start" : "end", fontSize: 10, fill: "#3A332D"})
    ]
  });
}
```
...
::: {.content-visible when-format="pdf"}
```{r}
#| label: fig-neto-pozicija-print
#| echo: false
#| fig-cap: "Neto proračunski položaj odabranih članica EU, euro po stanovniku, 2023. Pozitivno su neto primatelji (zeleno), negativno neto uplatitelji (crveno). Hrvatska (zlatno) je među najvećim neto primateljima po stanovniku. Izvor: Europska komisija; Deutsche Bundesbank, IW Köln."
#| fig-alt: "Vodoravni stupčasti graf neto položaja po stanovniku; Hrvatska +619 € i Rumunjska +307 € primatelji, Nizozemska −350 € uplatitelj."
#| fig-width: 7.8
#| fig-height: 2.9
source("R/setup.R")
theme_set(theme_pubfin() + theme(
...
np <- data.frame(
  z = c("Hrvatska","Rumunjska","Poljska","Francuska","Nizozemska"),
  v = c(619,307,191,-30,-350),
  grp = c("HR","prim","prim","upl","upl"),
  stringsAsFactors = FALSE
)
np$z <- factor(np$z, levels = rev(np$z))
ggplot(np, aes(v, z, fill = grp)) +
  geom_col(width = 0.7) +
  geom_vline(xintercept = 0, color = "#3A332D", linewidth = 0.5) +
  geom_text(aes(label = paste0(ifelse(v > 0, "+", ""), v, " €"),
                hjust = ifelse(v > 0, -0.15, 1.15)), size = 2.8, color = "#3A332D") +
  scale_fill_manual(values = c("HR"="#1A1A1A","prim"="#4D4D4D","upl"="#8C8C8C"), guide = "none") +
  scale_x_continuous(limits = c(-470, 760)) +
  labs(x = "Neto položaj, € po stanovniku (2023.)", y = NULL)
```
:::
...
Polazište pravila nalazi se u stvaranju Ekonomske i monetarne unije. Uvođenje zajedničke valute značilo je da članice gube vlastiti tečaj i monetarnu politiku kao instrumente prilagodbe, pa fiskalna politika postaje važnija, ali i potencijalno opasnija za ostatak unije. Ako država izgubi konkurentnost, više ne može devalvirati, ako uđe u recesiju, ne može sama smanjiti kamatne stope, a ako joj dug postane problematičan, nema vlastitu središnju banku kao oslonac za tržište obveznica [@degrauwe2022; @baldwinwyplosz2022].

...
grp_cols <- c("niži dohodak"    = "#C53030",
              "srednji dohodak" = "#C8942D",
              "visoki dohodak"  = "#2D5A8E")

ggplot() +
  # 1. trendna linija (isprekidana, topla siva)
  geom_line(data = tline, aes(gdp, sp),
            linetype = "dashed", linewidth = 0.55, color = "#6B6357") +
  ...
  geom_text(data = data, aes(gdp, sp, label = z),
            vjust = 0, nudge_y = 1.6, size = 3.1, color = "#3A332D") +
  # 4. hipotetska točka (veća, crna)
  geom_point(data = hyp, aes(gdp, sp),
             shape = 21, size = 5.5, fill = "#1C1916", color = "white", stroke = 0.9) +

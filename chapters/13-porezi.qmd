...
## Pokušajte sami: porezni klin u Hrvatskoj {#sec-porezni-klin-r}

Sljedeći blok izvršava se izravno u vašem pregledniku — pokrenite ga, izmijenite stope ili granice osobnog odbitka i pogledajte kako se mijenja porezni klin po razini bruto plaće. (Prvo pokretanje traje par sekundi dok se R učitava u memoriju preglednika.)

```{webr-r}
# Hrvatska, pojednostavljeni model — porezni klin po bruto plaći.
# Pretpostavke:
#   - Doprinosi radnika (mirovinsko):       20.0 %
#   - Doprinosi poslodavca (zdravstveno):   16.5 %
#   - Stopa poreza na dohodak:              23.0 %  (jedinstvena, post-2024)
#   - Osobni odbitak:                       600 €
#
# Porezni klin = (ukupni trošak rada − neto plaća) / ukupni trošak rada

bruto <- seq(800, 5000, by = 100)        # raspon mjesečnih bruto plaća
osobni_odbitak <- 600

doprinosi_radnik    <- 0.200 * bruto
doprinosi_poslodavac <- 0.165 * bruto
porezna_osnovica    <- pmax(0, bruto - doprinosi_radnik - osobni_odbitak)
porez_dohodak       <- 0.23 * porezna_osnovica

neto           <- bruto - doprinosi_radnik - porez_dohodak
ukupni_trosak  <- bruto + doprinosi_poslodavac
klin           <- (ukupni_trosak - neto) / ukupni_trosak

plot(bruto, 100 * klin, type = "l", lwd = 2,
     xlab = "Bruto plaća (€/mj.)",
     ylab = "Porezni klin (%)",
     main = "Hrvatska 2024 — porezni klin po razini plaće",
     ylim = c(25, 50))
abline(h = 38, lty = 2, col = "firebrick")
text(3500, 38.8, "Prosjek EU (~38 %)", col = "firebrick", pos = 3)
grid(col = "grey85")
```

::: {.callout-tip}
**Što isprobati.** Promijenite stopu poreza na dohodak s `0.23` na `0.30` (predreformsko stanje) — koliko se klin pomakne pri 1.500 €? Pa onda na `0.20` — koja je razlika u ponuđenom poticaju za rad? Promijenite osobni odbitak na 800 € i provjerite kako to utječe na klin u donjem dijelu raspona plaća. Ovaj jednostavni model već pokazuje zašto su politički izbori oko stope, odbitka i strukture doprinosa kvantitativno tako značajni.
:::

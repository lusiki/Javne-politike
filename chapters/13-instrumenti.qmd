...
Sustavnija klasifikacija instrumenata postoji u literaturi o javnom upravljanju i naziva se **NATO okvir** [@hood1983].

::: {#def-nato-okvir}
**NATO okvir** Christophera Hooda klasificira državne instrumente prema četiri vrste resursa koje vlada može koristiti, a to su *Nodality* (informacijska centralnost), *Authority* (formalna ovlast), *Treasure* (financijska moć) i *Organization* (vlastiti institucionalni kapacitet) [@hood1983].
:::
...
Prije nego što razložimo pojedine instrumente, vrijedi se vratiti na pitanje zašto država uopće poseže za njima. Instrumenti iz ovog poglavlja odgovaraju na neuspjehe tržišta opisane u prvom dijelu knjige, pa izbor instrumenta uvijek polazi od toga koji se problem rješava. @fig-trzisni-neuspjesi sažima tu vezu i grupira tržišne neuspjehe u tri velike skupine, neefikasnost, nejednakost i makroekonomsku nestabilnost, te uz svaku navodi tip intervencije kojim država najčešće odgovara. Prikaz je namjerno pojednostavnjen jer u praksi jedan instrument rijetko pripada samo jednoj skupini, ali pokazuje logiku koja povezuje dijagnozu problema s izborom alata.

![Glavne kategorije tržišnih neuspjeha i pripadni instrumenti državne intervencije. Prikaz je pojednostavnjen i služi za udžbeničko objašnjenje osnovnih funkcija države u gospodarstvu.](../images/infographics/13-trzisni-neuspjesi-intervencija.svg){#fig-trzisni-neuspjesi .infographic fig-alt="Dijagram koji povezuje tri kategorije tržišnih neuspjeha — neefikasnost, nejednakost i makroekonomske probleme — s tri tipa instrumenata državne intervencije." width=92%}
...
pa porez podiže privatni trošak za točno onaj iznos koji nedostaje da bi se odluka pojedinca poklopila s društveno poželjnom [@pigou1920]. Ključno je da se porez vezuje uz granični vanjski trošak procijenjen u optimumu $Q^{*}$, a ne u zatečenoj tržišnoj ravnoteži, jer upravo ta razina vraća proizvodnju na društveno najbolju količinu.
...
gdje su $\varepsilon_{S}$ i $\varepsilon_{D}$ elastičnosti ponude i potražnje [@harberger1954]. Prvi oblik se čita izravno s grafa kao površina osjenčanog trokuta, a drugi pokazuje zašto mrtvi teret raste otprilike s kvadratom jačine mjere, jer udvostručenje klina $t$ otprilike učetverostručuje gubitak. Isti izraz govori i da je trošak veći ondje gdje su ponuda i potražnja elastičnije, jer tada ista mjera obara količinu jače.

::: {.content-visible when-format="html"}
```{ojs}
//| echo: false
viewof reg_controls = Inputs.form({
  t: Inputs.range([0, 60], {value: 30, step: 1, label: "Jačina mjere (porez/klin) t:"})
})
```

```{ojs}
//| echo: false
//| label: fig-regulacija-mrtvi-teret
//| fig-cap: "Regulacija i mrtvi teret. Mjera razdvaja cijenu kupca i proizvođača za iznos t, obara količinu s Q* na Q_reg, a osjenčani trokut je vrijednost koja propada jer se manje razmjenjuje."
//| fig-alt: "Graf s padajućom krivuljom potražnje i rastućom krivuljom ponude koje se sijeku u konkurentskoj točki Q*. Mjera podiže cijenu kupca iznad i spušta cijenu proizvođača ispod ravnotežne, smanjuje količinu na Q_reg, a osjenčani trokut između Q_reg i Q* prikazuje mrtvi teret."
{
  const t = reg_controls.t;
  const D = (q) => 100 - q;
  const S = (q) => 20 + q;
  const Qstar = 40, Pstar = 60;
  const Qreg = 40 - t / 2;
  const Pc = D(Qreg), Pp = S(Qreg);
  const dwl = 0.5 * t * (Qstar - Qreg);
  const qs = d3.range(0, 81, 1);
  const demand = qs.map(q => ({q, p: D(q)})).filter(p => p.p >= 0);
  const supply = qs.map(q => ({q, p: S(q)})).filter(p => p.p <= 100);
  const tri = [{q: Qreg, p: Pc}, {q: Qreg, p: Pp}, {q: Qstar, p: Pstar}];
  ...
  return Plot.plot({
    ...
    y: {label: "↑ Cijena", domain: [0, 100], grid: true},
    marks: [
      ...
      Plot.ruleY([Pc], {x1: 0, x2: Qreg, stroke: "#6B6357", strokeDasharray: "4,3"}),
      Plot.ruleY([Pp], {x1: 0, x2: Qreg, stroke: "#6B6357", strokeDasharray: "4,3"}),
      Plot.ruleX([Qreg],  {y1: 0, y2: Pc,    stroke: "#6B6357", strokeDasharray: "4,3"}),
      ...
      Plot.text([{q: 74, y: S(74)}], {x: "q", y: "y", text: ["ponuda"], dy: -8, fill: "#4A6B5C", fontSize: 11, fontWeight: 600}),
      Plot.text([{q: 1, y: Pc}], {x: "x", y: "y", text: [`cijena kupca ${Pc.toFixed(0)}`], textAnchor: "start", dy: -6, fill: "#6B1F26", fontSize: 10}),
      Plot.text([{q: 1, y: Pp}], {x: "x", y: "y", text: [`cijena proizvođača ${Pp.toFixed(0)}`], textAnchor: "start", dy: 12, fill: "#4A6B5C", fontSize: 10}),
      ...
      Plot.text([`Klin t = ${t}      Mrtvi teret = ${dwl.toFixed(0)}`],
...
```{r}
#| label: fig-regulacija-mrtvi-teret-print
#| echo: false
#| fig-cap: "Regulacija i mrtvi teret. Mjera razdvaja cijenu kupca i proizvođača za iznos t, obara količinu s Q* na Q_reg, a osjenčani trokut je vrijednost koja propada jer se manje razmjenjuje."
#| fig-alt: "Graf s padajućom krivuljom potražnje i rastućom krivuljom ponude koje se sijeku u konkurentskoj točki Q*. Mjera podiže cijenu kupca iznad i spušta cijenu proizvođača ispod ravnotežne, smanjuje količinu na Q_reg, a osjenčani trokut između Q_reg i Q* prikazuje mrtvi teret."
#| fig-width: 7.6
#| fig-height: 4.8
# Statički PDF blizanac interaktivnog OJS grafa (zadana vrijednost: t = 30).
source("R/setup.R")
...
ggplot() +
  ...
  geom_segment(aes(x = 0, xend = Qreg, y = Pc, yend = Pc), linetype = "dashed", color = "#8C8C8C", linewidth = 0.4) +
  geom_segment(aes(x = 0, xend = Qreg, y = Pp, yend = Pp), linetype = "dashed", color = "#8C8C8C", linewidth = 0.4) +
  geom_segment(aes(x = Qreg,  xend = Qreg,  y = 0, yend = Pc),    linetype = "dashed", color = "#8C8C8C", linewidth = 0.4) +
  ...
  annotate("text", x = 74, y = S(74) + 4, label = "ponuda", color = "#7A7A7A", fontface = "bold", size = 3.2) +
  annotate("text", x = 1, y = Pc + 4, label = sprintf("cijena kupca %.0f", Pc), hjust = 0, color = "#4D4D4D", size = 3) +
  annotate("text", x = 1, y = Pp - 4, label = sprintf("cijena proizvođača %.0f", Pp), hjust = 0, color = "#7A7A7A", size = 3) +
  annotate("text", x = Qreg, y = 2, label = "Q_reg", color = "#8C8C8C", size = 3.2) +
  annotate("text", x = Qstar, y = 2, label = "Q*", color = "#8C8C8C", size = 3.2) +
  annotate("text", x = 2, y = 97, label = sprintf("Klin t = %d   Mrtvi teret = %.0f", t, dwl),
           hjust = 0, fontface = "bold", size = 4, color = "#000000") +
  scale_x_continuous(limits = c(0, 80), expand = c(0, 0)) +
  scale_y_continuous(limits = c(0, 100), expand = c(0, 0)) +
  labs(x = "Količina Q", y = "Cijena") +
  theme_pubfin()
```
:::
...
[^wp-ch12]: Daljnje čitanje: Murray i Rivers (2015), [*British Columbia's Revenue-Neutral Carbon Tax: A Review*](https://nicholasinstitute.duke.edu/climate/publications/british-columbias-revenue-neutral-carbon-tax-review-latest-grand-experiment) (NI WP 15-04).

...
//| echo: false
//| label: fig-euprava
//| fig-cap: "Indeks razvijenosti e-uprave (EGDI, ljestvica 0–1), odabrane zemlje, 2024. Zlatno je istaknuta Hrvatska, a isprekidana linija označava europski regionalni prosjek. Izvor: Ujedinjeni narodi (UN DESA), E-Government Survey 2024."
//| fig-alt: "Vodoravni stupčasti graf indeksa razvijenosti e-uprave za osam zemalja. Danska je na vrhu s 0,985, Francuska na dnu s 0,874. Hrvatska je istaknuta zlatno s 0,882, iznad europskog regionalnog prosjeka od 0,849."
{
  const data = [
    {z:"Danska", v:0.985, grp:"ostalo"},
    {z:"Estonija", v:0.973, grp:"ostalo"},
    {z:"Finska", v:0.958, grp:"ostalo"},
    {z:"Njemačka", v:0.942, grp:"ostalo"},
    {z:"Švedska", v:0.933, grp:"ostalo"},
    {z:"Hrvatska", v:0.882, grp:"HR"},
    {z:"Slovenija", v:0.876, grp:"ostalo"},
    {z:"Francuska", v:0.874, grp:"ostalo"}
  ];
  return Plot.plot({
    width: 780, height: 400, marginLeft: 86, marginRight: 56, marginTop: 28, marginBottom: 46,
    style: {fontSize: "12px", fontFamily: "Public Sans, system-ui, sans-serif", color: "#3A332D"},
    x: {label: "Indeks razvijenosti e-uprave (EGDI, 0–1) →", domain: [0, 1.0], grid: true},
    y: {label: null, domain: data.map(d => d.z)},
    color: {domain: ["ostalo","HR"], range: ["#4A5568","#C8942D"]},
    marks: [
      Plot.barX(data, {x: "v", y: "z", fill: "grp", tip: true}),
      Plot.ruleX([0.849], {stroke: "#8C8C8C", strokeDasharray: "5,4", strokeWidth: 1.5}),
      Plot.text([{z: "Danska"}], {x: 0.849, y: "z", text: ["Europa 0,849"], dy: -12, dx: 3, textAnchor: "start", fontSize: 10, fill: "#6B6357"}),
      Plot.text(data, {x: "v", y: "z", text: d => d.v.toFixed(3).replace(".", ","), dx: 4, textAnchor: "start", fontSize: 10, fill: "#3A332D"})
    ]
  });
}
```
...
```{r}
#| label: fig-euprava-print
#| echo: false
#| fig-cap: "Indeks razvijenosti e-uprave (EGDI, ljestvica 0–1), odabrane zemlje, 2024. Zlatno je istaknuta Hrvatska, a isprekidana linija označava europski regionalni prosjek. Izvor: Ujedinjeni narodi (UN DESA), E-Government Survey 2024."
#| fig-alt: "Vodoravni stupčasti graf indeksa razvijenosti e-uprave; Danska 0,985, Francuska 0,874, Hrvatska istaknuta zlatno s 0,882, iznad europskog prosjeka od 0,849."
#| fig-width: 7.8
#| fig-height: 4.0
source("R/setup.R")

eg <- data.frame(
  z = c("Danska","Estonija","Finska","Njemačka","Švedska","Hrvatska","Slovenija","Francuska"),
  v = c(0.985, 0.973, 0.958, 0.942, 0.933, 0.882, 0.876, 0.874),
  grp = c("ostalo","ostalo","ostalo","ostalo","ostalo","HR","ostalo","ostalo"),
  stringsAsFactors = FALSE
)
eg$z <- factor(eg$z, levels = rev(eg$z))

ggplot(eg, aes(v, z, fill = grp)) +
  geom_col(width = 0.72) +
  geom_vline(xintercept = 0.849, linetype = "dashed", color = "#8C8C8C", linewidth = 0.5) +
  annotate("text", x = 0.849, y = 8, label = "Europa 0,849", hjust = -0.05, vjust = 0,
           size = 2.8, color = "#6B6357") +
  geom_text(aes(label = sub("\\.", ",", sprintf("%.3f", v))),
            hjust = -0.15, size = 2.8, color = "#3A332D") +
  scale_fill_manual(values = c("ostalo" = "#4A5568", "HR" = "#C8942D"), guide = "none") +
  scale_x_continuous(limits = c(0, 1.05)) +
  labs(x = "Indeks razvijenosti e-uprave (EGDI, 0–1)", y = NULL) +
  theme_pubfin()
```
:::

...
Da bi pravila bila pravična, akteri koji ih biraju ne bi smjeli moći skrojiti ih prema vlastitoj trenutnoj koristi. Buchanan i Tullock zato ex ante naglasak grade na zamišljenom **velu neizvjesnosti** (*veil of uncertainty*), koji upravo tu mogućnost uklanja [@buchanan1962].
...
Buchanan i Tullock taj izbor pravila prikazuju kroz **račun pristanka**, usporedbu troškova koju svaki sudionik radi kada bira koliku suglasnost tražiti za kolektivne odluke [@buchanan1962; @mueller1989]. Označimo li s $k$ prag suglasnosti, broj sudionika čiji je pristanak nužan da bi odluka prošla, ukupni trošak kolektivnog odlučivanja zbroj je dviju komponenti koje se kreću u suprotnim smjerovima,
...
Ovaj su problem u modernoj makroekonomskoj literaturi posebno razvili Finn Kydland i Edward Prescott, pokazujući da diskrecijska politika često vodi do slabijih ishoda od politika temeljenih na vjerodostojnim pravilima [@kydland1977]. Suština problema je u tome da ono što je optimalno obećati za budućnost često nije optimalno provesti kada ta budućnost postane sadašnjost. Vlada može danas najaviti da će voditi odgovornu fiskalnu politiku, smanjivati deficit i kontrolirati javni dug. Takva najava može povećati povjerenje. Međutim, kada se približe izbori ili kada se pojave pritisci interesnih skupina, ista vlada može imati poticaj odstupiti od ranije najavljene politike. Dugoročno odgovorna politika tada postaje kratkoročno politički skupa.
...
```{ojs}
//| echo: false
//| label: fig-fiskalna-pravila
//| fig-cap: "Indeks snage nacionalnih fiskalnih pravila, odabrane članice EU (standardizirano oko prosjeka EU; više znači stroža i obvezujućija pravila). Vrijednosti su indikativne i odnose se na razdoblje oko 2012.–2013. (Hrvatska 2013.). Izvor: Europska komisija, baza fiskalnog upravljanja; izračun za Hrvatsku prema metodologiji EK."
//| fig-alt: "Vodoravni stupčasti graf indeksa snage fiskalnih pravila za deset članica EU. Španjolska je na vrhu s 3,3, Slovenija na dnu s −0,8. Hrvatska je istaknuta zlatno s 1,4, u gornjoj polovici, iznad okomite crte koja označava prosjek EU na nuli."
{
  const data = [
    {z:"Španjolska", v:3.3, grp:"ostalo"},
    {z:"Švedska", v:2.5, grp:"ostalo"},
    {z:"Bugarska", v:2.2, grp:"ostalo"},
    {z:"Poljska", v:1.9, grp:"ostalo"},
    {z:"Francuska", v:1.6, grp:"ostalo"},
    {z:"Hrvatska", v:1.4, grp:"HR"},
    {z:"Njemačka", v:1.4, grp:"ostalo"},
    {z:"Nizozemska", v:1.2, grp:"ostalo"},
    {z:"Italija", v:-0.2, grp:"ostalo"},
    {z:"Slovenija", v:-0.8, grp:"ostalo"}
  ];
  return Plot.plot({
    width: 780, height: 440, marginLeft: 92, marginRight: 50, marginTop: 28, marginBottom: 46,
    style: {fontSize: "12px", fontFamily: "Public Sans, system-ui, sans-serif", color: "#3A332D"},
    x: {label: "Indeks snage fiskalnih pravila (standardizirano) →", domain: [-1.5, 3.7], grid: true},
    y: {label: null, domain: data.map(d => d.z)},
    color: {domain: ["ostalo","HR"], range: ["#4A5568","#C8942D"]},
    marks: [
      Plot.barX(data, {x: "v", y: "z", fill: "grp", tip: true}),
      Plot.ruleX([0], {stroke: "#6B6357", strokeWidth: 1.2}),
      Plot.text([{z: "Španjolska"}], {x: 0, y: "z", text: ["prosjek EU"], dy: -12, dx: 3, textAnchor: "start", fontSize: 10, fill: "#6B6357"}),
      Plot.text(data, {x: "v", y: "z", text: d => d.v.toFixed(1).replace(".", ","), dx: d => d.v >= 0 ? 4 : -4, textAnchor: d => d.v >= 0 ? "start" : "end", fontSize: 10, fill: "#3A332D"})
    ]
  });
}
```
...
```{r}
#| label: fig-fiskalna-pravila-print
#| echo: false
#| fig-cap: "Indeks snage nacionalnih fiskalnih pravila, odabrane članice EU (standardizirano oko prosjeka EU; više znači stroža pravila). Vrijednosti indikativne, oko 2012.–2013. (Hrvatska 2013.). Izvor: Europska komisija, baza fiskalnog upravljanja; izračun za Hrvatsku prema metodologiji EK."
#| fig-alt: "Vodoravni stupčasti graf indeksa snage fiskalnih pravila; Španjolska 3,3, Slovenija −0,8, Hrvatska istaknuta zlatno s 1,4 u gornjoj polovici."
#| fig-width: 7.8
#| fig-height: 4.4
source("R/setup.R")

fr <- data.frame(
  z = c("Španjolska","Švedska","Bugarska","Poljska","Francuska","Hrvatska","Njemačka","Nizozemska","Italija","Slovenija"),
  v = c(3.3, 2.5, 2.2, 1.9, 1.6, 1.4, 1.4, 1.2, -0.2, -0.8),
  grp = c("ostalo","ostalo","ostalo","ostalo","ostalo","HR","ostalo","ostalo","ostalo","ostalo"),
  stringsAsFactors = FALSE
)
fr$z <- factor(fr$z, levels = rev(fr$z))

ggplot(fr, aes(v, z, fill = grp)) +
  geom_col(width = 0.72) +
  geom_vline(xintercept = 0, color = "#6B6357", linewidth = 0.5) +
  geom_text(aes(label = sub("\\.", ",", sprintf("%.1f", v)), hjust = ifelse(v >= 0, -0.2, 1.2)),
            size = 2.9, color = "#3A332D") +
  scale_fill_manual(values = c("ostalo" = "#8C8C8C", "HR" = "#1A1A1A"), guide = "none") +
  scale_x_continuous(limits = c(-1.5, 3.7)) +
  labs(x = "Indeks snage fiskalnih pravila (standardizirano)", y = NULL) +
  theme_pubfin()
```
:::
...
::: {#def-politicko-proracunski-ciklus}
**Političko-proračunski ciklus** je sustavno prilagođavanje fiskalne politike izbornom kalendaru, u kojemu se rashodi povećavaju a porezi smanjuju pred izbore radi veće potpore birača, dok se prilagodba i troškovi odgađaju na razdoblje nakon izbora [@nordhaus1975].
:::
...
Da taj uvjet nije puka pretpostavka potvrđuje način na koji je ciklus izmjeren. Usporedne studije razdvajaju zemlje prema demokratskoj zrelosti i nalaze da se ciklus gotovo u cijelosti koncentrira u mladim demokracijama, dok u dugotrajnima uglavnom izostaje, čime se Nordhausovo izvorno predviđanje univerzalnog ciklusa pokazuje preuskim [@brender2005; @shi2006]. Ta usporedba ujedno identificira mehanizam, jer ono što mlade demokracije razlikuje nije priroda političara nego nedostatak biračkog iskustva u prepoznavanju i kažnjavanju predizbornog popuštanja. Ondje gdje birači još ne nadziru ciklus, tu ulogu moraju preuzeti pravila i neovisne institucije koje buduće troškove čine vidljivima, što je upravo ono što ustavna ograničenja trebaju postići.
...
::: {#def-vjerodostojnost}
**Vjerodostojnost** je svojstvo politike ili pravila za koje akteri vjeruju da će biti provedeno i u trenutku kada to vlastima više neće odgovarati; gradi se kroz dosljednost, reputaciju, neovisne institucije i transparentnost, a ne pukom najavom, i upravo ona daje pravilima moć da utječu na očekivanja [@kydland1977].
:::

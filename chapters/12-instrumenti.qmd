...
ggplot() +
  ...
  geom_segment(aes(x = 0, xend = Qreg, y = Pc, yend = Pc), linetype = "dashed", color = "#6B6357", linewidth = 0.4) +
  geom_segment(aes(x = 0, xend = Qreg, y = Pp, yend = Pp), linetype = "dashed", color = "#6B6357", linewidth = 0.4) +
  geom_segment(aes(x = Qreg,  xend = Qreg,  y = 0, yend = Pc),    linetype = "dashed", color = "#6B6357", linewidth = 0.4) +
  ...
  annotate("text", x = 74, y = S(74) + 4, label = "ponuda", color = "#4A6B5C", fontface = "bold", size = 3.2) +
  annotate("text", x = 1, y = Pc + 4, label = sprintf("cijena kupca %.0f", Pc), hjust = 0, color = "#6B1F26", size = 3) +
  annotate("text", x = 1, y = Pp - 4, label = sprintf("cijena proizvođača %.0f", Pp), hjust = 0, color = "#4A6B5C", size = 3) +
...
Instrumenti javne politike na taj način predstavljaju most između državnih ciljeva i stvarnih društvenih ishoda. Država može imati plemenit cilj poput smanjenja siromaštva, povećanja zaposlenosti, zaštite okoliša ili poticanja rasta, ali cilj sam po sebi nije politika. Politika nastaje tek kada se odabere instrument, osigura provedba, definiraju odgovornosti i mjere rezultati. Dobra javna politika zato ne pita samo što država želi postići, nego i kojim instrumentom, uz koje troškove, s kakvim poticajima, preko kojih institucija i s kakvom mogućnošću evaluacije.

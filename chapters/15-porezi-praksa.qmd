...
cols <- c("ostalo" = "#8C8C8C", "OECD" = "#4D4D4D", "HR" = "#1A1A1A")
...
cols <- c("Potrošnja" = "#1A1A1A", "Dohodak i dobit" = "#5C5C5C", "Doprinosi" = "#A0A0A0")
...
cols <- c("Neizravni (potrošnja)" = "#1A1A1A",
          "Izravni (dohodak i kapital)" = "#5C5C5C",
          "Socijalni doprinosi" = "#A0A0A0")
tot <- data.frame(
...
ggplot(ps, aes(val, z, fill = cat)) +
  ...
  geom_text(data = tot, aes(t, z, label = sub("\\.", ",", sprintf("%.1f", t))),
            color = ifelse(tot$hl, "#1A1A1A", "#3A332D"),
...
ggplot(df, aes(val, z, fill = cat)) +
  ...
  scale_fill_manual(values = c("Doprinosi poslodavca"="#333333","Doprinosi zaposlenika"="#7A7A7A"), name = NULL) +
  scale_color_manual(values = c(`TRUE`="#1A1A1A", `FALSE`=NA), guide = "none") +
...
ggplot(kl, aes(v, z, fill = grp)) +
  ...
  scale_fill_manual(values = c("ostalo"="#8C8C8C","OECD"="#4D4D4D","HR"="#1A1A1A"), guide = "none") +
...
cols <- c("EU" = "#7A7A7A", "Hrvatska" = "#1A1A1A")

...
df$seg_boja <- ifelse(df$zemlja == "Hrvatska", "#1A1A1A", "#C9C3B8")
df$seg_sirina <- ifelse(df$zemlja == "Hrvatska", 1.9, 1.3)

y_lab_boje <- ifelse(levels(df$zemlja) == "Hrvatska", "#1A1A1A", "#3A332D")
...
ggplot(df) +
  ...
  geom_text(aes(x = gustoca, y = zemlja, label = sub("\\.", ",", sprintf("%.1f", gustoca))),
            vjust = -1.1, size = 2.6, color = "#4D4D4D") +
  geom_text(aes(x = pokrivenost, y = zemlja, label = sub("\\.", ",", sprintf("%.1f", pokrivenost))),
            vjust = -1.1, size = 2.6, color = "#7A7A7A") +
  scale_color_manual(values = c("Sindikalna gustoća" = "#333333",
                                "Pokrivenost kolektivnim ugovorima" = "#8C8C8C"), name = NULL) +

...
ggplot(df, aes(x, d)) +
  geom_area(data = subset(df, side == "A"), fill = "#4A6B5C", alpha = 0.35) +
  geom_area(data = subset(df, side == "B"), fill = "#6B1F26", alpha = 0.35) +
  geom_line(color = "#1C1916", linewidth = 0.7) +
  geom_vline(xintercept = 0.5, linetype = "dashed", color = "#6B6357", linewidth = 0.4) +
  geom_vline(xintercept = pA, color = "#4A6B5C", linewidth = 1.0) +
  geom_vline(xintercept = pB, color = "#6B1F26", linewidth = 1.0) +
  annotate("text", x = pA, y = 1.05, label = "A", color = "#4A6B5C", fontface = "bold", size = 4.5) +
  annotate("text", x = pB, y = 1.05, label = "B", color = "#6B1F26", fontface = "bold", size = 4.5) +
  annotate("text", x = 0.5, y = 1.05, label = "medijan", color = "#6B6357", size = 3) +
  annotate("text", x = 0.5, y = 1.15,
           label = sprintf("Stranka A %.0f %%   Stranka B %.0f %%   pobjednik: %s", shareA*100, shareB*100, winner),
           fontface = "bold", size = 4, color = "#1C1916") +
...
ggplot(df, aes(p, s)) +
  ...
  geom_segment(aes(x = pStar, xend = pStar, y = 0, yend = sMax),
               linetype = "dashed", linewidth = 0.4, color = "#6B6357") +
  geom_point(aes(x = pStar, y = sMax), size = 3, shape = 21,
             fill = "#6B1F26", color = "white", stroke = 1) +
  annotate("text", x = pStar, y = sMax, label = sprintf("vrh pri %.1f obećanja", pStar),
           vjust = -1, fontface = "bold", size = 3.8, color = "#2D5A8E") +
...
Tvrdnja da se stranke razmiču ili da je njihova distribucija postala bimodalna mjerni je iskaz koji počiva na procjeni stranačkih pozicija iz obrazaca glasovanja, postupku prostornog skaliranja o kojem govori poglavlje o kolektivnom izboru.

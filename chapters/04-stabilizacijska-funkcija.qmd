...
Kako država tu zadaću obavlja, poglavlje razlaže korak po korak, od fiskalnog multiplikatora i automatskih stabilizatora do dosega i opasnosti diskrecijskih mjera. Na kraju se sama stabilnost pokazuje kao javno dobro koje tržište ne osigurava samo, čime se zatvara luk triju Musgraveovih funkcija.
...
ggplot() +
  ...
  theme(panel.grid.major.x = element_blank(),
        ...
        strip.text = element_text(face = "bold", color = "#1C1916"))
```
:::
...
ggplot(df, aes(t, u)) +
  geom_hline(yintercept = un, linetype = "dashed", linewidth = 0.5, color = "#6B6357") +
  geom_vline(xintercept = c(1, 2), linewidth = 0.5, color = "#6B1F26") +
  geom_line(linewidth = 1.1, color = "#2D5A8E", na.rm = TRUE) +
  annotate("text", x = 1, y = un + A + 0.7, label = "izbori",
           color = "#6B1F26", fontface = "bold", size = 3.4) +
  annotate("text", x = 2, y = un + A + 0.7, label = "izbori",
           color = "#6B1F26", fontface = "bold", size = 3.4) +
  annotate("text", x = 0.02, y = un, label = "prirodna stopa", hjust = 0, vjust = -0.6,
           color = "#6B6357", size = 3.0) +

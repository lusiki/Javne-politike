...
//| echo: false
{
  ...
  const ae0 = grid.map(y => ({Y: y, AE: A0 + c * y})).filter(p => p.AE <= Ymax);
  const aeG = grid.map(y => ({Y: y, AE: A0 + dG + c * y})).filter(p => p.AE <= Ymax);
  const aeT = grid.map(y => ({Y: y, AE: A0 - c * dT + c * y}))
                  .filter(p => p.AE <= Ymax && p.AE >= 0);
  const line45 = [{Y: 0, AE: 0}, {Y: Ymax, AE: Ymax}];

  const commonStyle = {fontSize: "12px", fontFamily: "Public Sans, system-ui, sans-serif", color: "#3A332D"};

  const panelG = Plot.plot({
    width: 430,
    height: 460,
    marginLeft: 60,
    marginBottom: 50,
    marginTop: 45,
    style: commonStyle,
    title: "Povećanje državne potrošnje",
    ...
    marks: [
      ...
      Plot.line(line45, {x: "Y", y: "AE", stroke: "#3A332D", strokeWidth: 1.5}),
      Plot.line(ae0, {x: "Y", y: "AE", stroke: "#2D5A8E", strokeWidth: 2.5}),
      Plot.line(aeG, {x: "Y", y: "AE", stroke: "#2D5A8E", strokeWidth: 2.5, strokeDasharray: "6,4"}),

      Plot.ruleX([Y0],  {y1: 0, y2: Y0,  stroke: "#6B6357", strokeDasharray: "4,3"}),
      Plot.ruleX([Y1G], {y1: 0, y2: Y1G, stroke: "#6B6357", strokeDasharray: "4,3"}),

      Plot.dot([{Y: Y0,  AE: Y0}],
        {x: "Y", y: "AE", r: 5, fill: "#2D5A8E", stroke: "white", strokeWidth: 2}),
      Plot.dot([{Y: Y1G, AE: Y1G}],
        {x: "Y", y: "AE", r: 5, fill: "#2D5A8E", stroke: "white", strokeWidth: 2}),

      Plot.text([{Y: Ymax * 0.92, AE: Ymax * 0.92, label: "45°"}],
        {x: "Y", y: "AE", text: "label", fill: "#3A332D", fontSize: 11, dx: 6, dy: -6}),
      Plot.text([{Y: Y0, AE: 10, label: `Y₀ = ${Y0.toFixed(0)}`}],
        {x: "Y", y: "AE", text: "label", textAnchor: "middle", fontSize: 11, fill: "#2D5A8E"}),
      Plot.text([{Y: Y1G, AE: 10, label: `Y₁ = ${Y1G.toFixed(0)}`}],
        {x: "Y", y: "AE", text: "label", textAnchor: "middle", fontSize: 11, fill: "#2D5A8E"}),
      Plot.text([{Y: 30, AE: Ymax * 0.96,
        label: `Multiplikator ΔY/ΔG = ${multG.toFixed(2)}`}],
        {x: "Y", y: "AE", text: "label", textAnchor: "start",
         fontSize: 13, fill: "#1C1916", fontWeight: 700})
    ]
  });

  const panelT = Plot.plot({
    width: 430,
    height: 460,
    marginLeft: 60,
    marginBottom: 50,
    ...
    marks: [
      ...
      Plot.line(line45, {x: "Y", y: "AE", stroke: "#3A332D", strokeWidth: 1.5}),
      Plot.line(ae0, {x: "Y", y: "AE", stroke: "#2D5A8E", strokeWidth: 2.5}),
      Plot.line(aeT, {x: "Y", y: "AE", stroke: "#2D5A8E", strokeWidth: 2.5, strokeDasharray: "6,4"}),

      Plot.ruleX([Y1T], {y1: 0, y2: Y1T, stroke: "#6B6357", strokeDasharray: "4,3"}),
      Plot.ruleX([Y0],  {y1: 0, y2: Y0,  stroke: "#6B6357", strokeDasharray: "4,3"}),

      Plot.dot([{Y: Y0,  AE: Y0}],
        {x: "Y", y: "AE", r: 5, fill: "#2D5A8E", stroke: "white", strokeWidth: 2}),
      Plot.dot([{Y: Y1T, AE: Y1T}],
        {x: "Y", y: "AE", r: 5, fill: "#2D5A8E", stroke: "white", strokeWidth: 2}),

      Plot.text([{Y: Ymax * 0.92, AE: Ymax * 0.92, label: "45°"}],
        {x: "Y", y: "AE", text: "label", fill: "#3A332D", fontSize: 11, dx: 6, dy: -6}),
      Plot.text([{Y: Y1T, AE: 10, label: `Y₁ = ${Y1T.toFixed(0)}`}],
        {x: "Y", y: "AE", text: "label", textAnchor: "middle", fontSize: 11, fill: "#2D5A8E"}),
      Plot.text([{Y: Y0, AE: 10, label: `Y₀ = ${Y0.toFixed(0)}`}],
        {x: "Y", y: "AE", text: "label", textAnchor: "middle", fontSize: 11, fill: "#2D5A8E"}),
      Plot.text([{Y: 30, AE: Ymax * 0.96,
        label: `Multiplikator ΔY/ΔT = ${multT.toFixed(2)}`}],
        {x: "Y", y: "AE", text: "label", textAnchor: "start",
         fontSize: 13, fill: "#1C1916", fontWeight: 700})
    ]
  });

  return htl.html`<div style="display:flex; gap:1rem; flex-wrap:wrap; justify-content:center;">${panelG}${panelT}</div>`;
}
```
